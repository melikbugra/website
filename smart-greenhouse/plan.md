...
## 1. Proje Özeti
Bu proje, kapalı cam balkon içerisinde 120x80x100 cm boyutlarında izole bir polikarbonat sera inşa edilmesini ve bu seranın iklim/sulama koşullarının Raspberry Pi & Hetzner sunucu mimarisiyle otonom olarak yönetilmesini amaçlamaktadır.
...
* **Kontrolcü:** Raspberry Pi.
* **Sensörler:**
    * DHT22 (Ortam Sıcaklık ve Nem Sensörü) - Seranın üst orta kısmına konumlandırılmalı.
    * Kapasitif Toprak Nemi Sensörleri (Korozyon direnci için kesinlikle kapasitif kullanılmalı).
* **Eylemciler (Aktüatörler):**
    * Çoklu Röle Kartı (5V tetiklemeli).
    * 12V Mini Dalgıç Su Pompası (Sulama rezervuarı için).
* **Güç Yönetimi:** 12V adaptör (Fanlar ve pompa için) ve step-down konvertör (Raspberry Pi'yi beslemek için).
...
### 3.1. Raspberry Pi (Edge)
* **Dil:** Python (`gpiozero` veya `RPi.GPIO` kütüphaneleri).
* **Görev:** Sensör verilerini saniyede/dakikada bir okumak, röleleri (fan, pompa) açıp kapatmak.
* **Haberleşme:** Okunan verileri ve donanım durumlarını (state) MQTT protokolü ile Hetzner sunucusuna iletmek.

### 3.2. Hetzner Sunucusu (Cloud)
* **Veri Alımı:** Eclipse Mosquitto (MQTT Broker).
* **Veritabanı:** InfluxDB (Zaman serisi verilerini kaydetmek için).
* **Görselleştirme:** Grafana (Sıcaklık, nem, toprak ıslaklığı ve fan/pompa çalışma sürelerini anlık izlemek için dashboard).
* **Gelişmiş Kontrol (Opsiyonel):** Sistemin kontrolünde standart eşik değerleri (if-else) yerine, ilerleyen aşamalarda Python ile sunucu tarafında çalışacak bir pekiştirmeli öğrenme (RL) modeli kurgulanabilir. Bu model, enerji tüketimini ve sıcaklık dalgalanmalarını minimize ederek bitkiler için en optimal politikayı (policy) öğrenebilir.
...
* **Faz 1:** Ahşap iskeletin kurulması ve polikarbonat kaplamanın yapılması.
* **Faz 2:** Fanların, Raspberry Pi'nin ve sensörlerin kabine fiziksel montajı.
* **Faz 3:** Python sensör okuma ve röle tetikleme kodlarının yazılması, MQTT ile Hetzner - InfluxDB - Grafana veri hattının kurulması.
